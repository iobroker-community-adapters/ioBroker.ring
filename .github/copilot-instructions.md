...
### Ring Adapter Context

This is the **Ring Adapter** for ioBroker, which integrates Ring security devices (doorbells, cameras, alarms) with the ioBroker smart home platform.

**Primary Function:** Connects to Ring's cloud API to provide:
- Real-time device status monitoring
- Motion and doorbell event notifications
- Live video streaming and snapshot capture
- Two-way audio communication
- Device control (siren activation, light control)

**Key Dependencies:**
- `ring-client-api`: Main library for Ring cloud API communication
- `ffmpeg-static`: Video processing for livestreams and snapshots
- `sharp`: Image processing for snapshots and overlays
- `rxjs`: Reactive programming for event handling

**Configuration Requirements:**
- Ring account refresh token (obtained through Ring's 2FA process)
- Video/snapshot storage paths for recordings
- Event filtering and automation settings
- Network timeout and polling configurations

**Device Types Supported:**
- Doorbell devices (various models including v1, v2, v3, pro series)
- Stick-up cameras (indoor, outdoor, battery, wired)
- Security cameras (floodlight, spotlight variations)
- Ring Intercom devices
- Alarm system components

**Unique Requirements:**
- Handles Ring's token-based authentication with automatic refresh
- Manages video recording and image processing workflows
- Implements rate limiting to respect Ring API quotas
- Provides snapshot overlay capabilities for timestamps and device info
...
### Ring Adapter Specific Patterns

**Authentication Pattern:**
```typescript
// Use RingApiClient wrapper for all Ring API interactions
...
```typescript
// Follow Ring device naming conventions
await this.setStateAsync(`devices.${deviceId}.info.device_name`, {val: device.name, ack: true});
await this.setStateAsync(`devices.${deviceId}.motion.motion_detected`, {val: motionDetected, ack: true});
```

**File Service Integration:**
```typescript
// Use FileService for all file operations
await FileService.prepareFolder(this.config.path_snapshot);
const filePath = FileService.generateFileName(this.config.filename_snapshot, deviceId);
```

**Event Handling:**
```typescript
// Subscribe to Ring events using rxjs observables
device.events.subscribe((event) => {
    this.processRingEvent(event, device);
});
```

**Logging Conventions:**
- Use `this.log.debug()` for API communication details
- Use `this.log.info()` for device discovery and major operations  
- Use `this.log.warn()` for recoverable API errors or configuration issues
- Use `this.log.error()` for authentication failures or critical errors

**Error Handling:**
- Always validate refresh token before API operations
- Implement retry logic for transient Ring API failures
