        ...
        if (location.cameras) {
          for (let j in location.cameras) {
            ...
            if (camera && camera.deviceType) {
              ...
              if (typeof camera.deviceType === 'string' && camera.deviceType.startsWith('doorbell')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (typeof camera.deviceType === 'string' && camera.deviceType.startsWith('jbox')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (typeof camera.deviceType === 'string' && camera.deviceType.startsWith('cocoa')) {
                this.setKind(camera.id, 'cocoa');
                alls.push(camera);
                continue;
              }
              if (typeof camera.deviceType === 'string' && camera.deviceType.startsWith('stickup')) {
