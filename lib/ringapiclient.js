...
class RingApiClient {
  ...
  async getLiveStream(id) {
    if (Date.now() - this.running < 5 * 10 * 1000) return;
    ...
    for (let step = 0; step < 5; step++) {
      try {
        ...
        this.adapter.log.debug('Sarting LiveStream for ' + recordtime + ' sec.');
        await doorbell.recordToFile(file, recordtime);
        this.adapter.log.debug('Stopping LiveStream!');
        ...
        this.adapter.log.debug('Error making LiveStream!');
