...
class RingApiClient {
  ...
  async getLiveStream(id, file) {
    try {
      ...
      for (let step = 0; step < maxstep; step++) {
        ...
        this.adapter.log.info('Starting LiveStream for ' + recordtime + ' sec.');
  ...
  async getSnapshot(id, file) {
    try {
      ...
      let doorbell = await this.getAllRingsDeviceObject(id);
      let sip = await doorbell.createSipSession();
      this.adapter.log.info('Starting Snapshot');
      let image = await doorbell.getSnapshot();
      this.adapter.log.info('Stopping Snapshot');
      if (image) {
      ...
      throw new Error('Could not get Snapshot for ' + id + ' in getSnapshot(). ' + error);
    }
    return null;
  }
