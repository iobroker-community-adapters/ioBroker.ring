...
class RingApiClient {
        ...
        if (location.cameras) {
          for (let j in location.cameras) {
            ...
            if (camera) {
              ...
              this.adapter.log.debug('Device: ' + JSON.stringify(camera.data));
              if (camera && camera.isDoorbot) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('doorbell')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('jbox')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('hp_cam')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('floodlight')) {
                this.setKind(camera.id, 'doorbell');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('cocoa')) {
                this.setKind(camera.id, 'cocoa');
                alls.push(camera);
                continue;
              }
              if (camera.deviceType && typeof camera.deviceType === 'string' && camera.deviceType.startsWith('stickup')) {
                this.setKind(camera.id, 'stickup');
                alls.push(camera);
                continue;
              }
  ...
  async getLiveStream(id, file) {
    try {
      ...
      for (let step = 0; step < maxstep; step++) {
        ...
        this.adapter.log.info('Starting creating livestream with recording time ' + recordtime + ' sec.');
        await doorbell.recordToFile(file, recordtime);
        if (fs.existsSync(file)) {
          this.adapter.log.info('Done creating livestream');
  ...
  async getSnapshot(id, file) {
    try {
      ...
      let doorbell = await this.getAllRingsDeviceObject(id);
      let sip = await doorbell.createSipSession();
      this.adapter.log.info('Starting creating snapshot');
      let image = await doorbell.getSnapshot();
      if (image) {
        this.adapter.log.info('Done creating snapshot');
        this.adapter.writeFile(file, image);
  ...
  async setLight(id, value) {
    try {
      ...
      if (camera) {
        ...
        this.adapter.log.info(`Set Light to ${value} for Id ${id}`);
      }
    } catch (error) {
