...
class Doorbell {
  ...
  async getDevices() {
    let devices;
    try {
      this.adapter.log.debug('Starting getDevices');
  ...
  async getAllRingsDevices() {
    try {
      ...
      for (let i in devices.doorbots) {
        ...
        this.adapter.log.debug('Doorbots: ' + JSON.stringify(devices.doorbots[i]));
      }
  ...
  async getSnapshot(id) {
    ...
    try {
      ...
      let image;
      if (id) {
        image = await this.ring.snapshot(id);
  ...
  async getHistory(id) {
    try {
      let histories = [];
      this.adapter.log.debug('Starting getHistory');
      let ring = await this.getRing();
  ...
  async setLight(id, value) {
    try {
      this.adapter.log.debug('Starting setLight');
  ...
  _event() {
    const check = async () => {
      try {
        ...
        this.adapter.log.debug('Starting _event');
        let ring = await this.getRing();
        if (ring) {
