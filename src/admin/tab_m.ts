...
function asyncEmit(command: any, data1 = "", data2 = ""): Promise<any> {
  return new Promise(function (resolve, reject) {
    if (data2 !== "")
      socket.emit(command, data1, data2, function (err:any, result:any) {
        if(!err && result)
          resolve(result)
        else
          reject(err)
      ...
      socket.emit(command , data1, function (err: any, result:any) {
        if(!err && result)
          resolve(result)
        else
          reject(err)
...
async function AddCams(): Promise<void> {
