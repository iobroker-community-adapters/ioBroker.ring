import type { IntercomHandsetData, RingIntercom } from 'ring-client-api' with { 'resolution-mode': 'import' };
...
export class OwnRingIntercom extends OwnRingDevice {
    ...
    public constructor(
        ...
        super(
            ...
            adapter,
            apiClient,
            OwnRingDevice.evaluateKind(ringDevice.deviceType, adapter, ringDevice),
            `${ringDevice.id}`,
    ...
    protected async recreateDeviceObjectTree(): Promise<void> {
        ...
        await this._adapter.upsertState(`${this.infoChannelId}.id`, COMMON_INFO_ID, this._ringIntercom.data.device_id);
        await this._adapter.upsertState(`${this.infoChannelId}.kind`, COMMON_INFO_KIND, this._ringIntercom.data.kind);
        ...
        await this._adapter.upsertState(`${this.eventsChannelId}.ding`, COMMON_EVENTS_INTERCOM_DING, false);
