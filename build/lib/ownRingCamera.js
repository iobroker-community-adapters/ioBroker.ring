...
class OwnRingCamera extends ownRingDevice_1.OwnRingDevice {
    ...
    _lastLightCommand = 0;
    _lastLiveStreamUrl = '';
    _lastLiveStreamTimestamp = 0;
    _lastSnapShotUrl = '';
    _lastSnapshotTimestamp = 0;
    _snapshotCount = 0;
    _lastHDSnapShotUrl = '';
    _lastHDSnapshotTimestamp = 0;
    _HDsnapshotCount = 0;
    _liveStreamCount = 0;
    _lastLiveStreamDir = '';
    _lastSnapShotDir = '';
    ...
    constructor(ringDevice, location, adapter, apiClient) {
        ...
        this._motionEventBlocker = new event_blocker_1.EventBlocker(this._adapter, this._adapter.config.ignore_events_Motion, this._adapter.config.keep_ignoring_if_retriggered);
        this._notifyEventBlocker = new event_blocker_1.EventBlocker(this._adapter, this._adapter.config.ignore_events_Motion, this._adapter.config.keep_ignoring_if_retriggered);
        this._doorbellEventBlocker = new event_blocker_1.EventBlocker(this._adapter, this._adapter.config.ignore_events_Doorbell, this._adapter.config.keep_ignoring_if_retriggered);
        ...
        this.recreateDeviceObjectTree().catch(e => this.catcher("Couldn't create Device Object Tree.", e));
        this.updateDeviceInfoObject(ringDevice.data).catch(e => this.catcher("Couldn't update Device Info Object.", e));
        this.updateHealth();
        this.updateHistory().catch(e => this.catcher("Couldn't update History.", e));
        this.autoSched();
        this.subscribeToEvents().catch(e => this.catcher("Couldn't subscribe to events.", e));
    }
    async startLivestream(duration) {
        ...
        const liveCall = await this._ringDevice
            .streamVideo({
        ...
        const liveCallSucceeded = await rxjs
            .firstValueFrom(liveCall.onCallEnded)
            .then((_result) => {
            return true;
        })
            .catch((reason) => {
    ...
    async toggleSiren(state) {
        ...
        try {
            ...
            await this._adapter.upsertState(`${this.sirenChannelId}.${constants_1.STATE_ID_SIREN_SWITCH}`, constants_1.COMMON_SIREN_SWITCH, state, true);
    ...
    async takeSnapshot(uuid, eventBased = false) {
        ...
        const image = await this._ringDevice
            .getNextSnapshot({ force: true, uuid: uuid })
            .then((result) => result)
            .catch((err) => {
            if (eventBased) {
            ...
            this.updateSnapshotRequest(false).catch(err => this.catcher('Cannot update snapshot request.', err));
            return err;
        });
        if (!image.byteLength) {
            if (eventBased) {
        ...
        if (this._adapter.config.overlay_snapshot) {
            image_txt = await image_service_1.ImageService.addTextToJpgBuffer(image, this._ringDevice.data.description, (0, strftime_1.default)(`${text_service_1.TextService.getTodayName(this._adapter.language)}, ${text_service_1.TextService.getDateFormat(this._adapter.dateFormat)} %T`)).catch((reason) => {
    ...
    async updateHistory() {
        ...
        await this._ringDevice.getEvents({ limit: 50 }).then(async (r) => {
            ...
            await this.updateHistoryObject(this.lastAction);
        });
    }
        ...
        switch (channelID) {
            ...
            case 'Siren':
                ...
                if (stateID === constants_1.STATE_ID_SIREN_SWITCH) {
                    ...
                    const success = await this._ringDevice.setSiren(targetVal);
                    if (success) {
                        await this._adapter.upsertState(`${this.sirenChannelId}.${constants_1.STATE_ID_SIREN_SWITCH}`, constants_1.COMMON_SIREN_SWITCH, targetVal, true);
                    }
                }
                else {
            ...
            case 'Light':
                ...
                if (stateID === constants_1.STATE_ID_LIGHT_SWITCH) {
                    ...
                    const success = await this._ringDevice.setLight(targetVal);
                    if (success) {
                        await this._adapter.upsertState(`${this.lightChannelId}.light_state`, constants_1.COMMON_LIGHT_STATE, targetVal);
                        await this._adapter.upsertState(`${this.lightChannelId}.light_switch`, constants_1.COMMON_LIGHT_SWITCH, targetVal, true);
                        this._adapter.setTimeout(() => void this.updateHealth(), 65000);
                    }
                }
                else {
                    this.error(`Unknown State/Switch with channel "${channelID}" and state "${stateID}"`);
                ...
                if (stateID === constants_1.STATE_ID_SNAPSHOT_REQUEST) {
                    ...
                    if (targetVal) {
                        await this.takeSnapshot().catch((reason) => {
                            this.updateSnapshotRequest(false).catch(e => this.catcher('Cannot update snapshot request', e));
                ...
                if (stateID === constants_1.STATE_ID_HDSNAPSHOT_REQUEST) {
                    ...
                    if (targetVal) {
                        await this.takeHDSnapshot().catch((reason) => {
                            this.updateHDSnapshotRequest(false).catch(e => this.catcher('Cannot update HD snapshot request', e));
                ...
                if (stateID === constants_1.STATE_ID_LIVESTREAM_REQUEST) {
                    ...
                    if (targetVal) {
                        await this.startLivestream().catch((reason) => {
                            this.updateLivestreamRequest(false).catch(e => this.catcher('Cannot update Livestream request', e));
    ...
    async recreateDeviceObjectTree() {
        ...
        if (this._ringDevice.hasSiren) {
            ...
            await this._adapter.setObjectNotExistsAsync(`${this.fullId}.${constants_1.CHANNEL_NAME_SIREN}`, {
                type: 'channel',
                common: {
        ...
        if (this._ringDevice.hasLight) {
            ...
            // Ersetzen von createChannel durch setObjectNotExistsAsync
            await this._adapter.setObjectNotExistsAsync(`${this.fullId}.${constants_1.CHANNEL_NAME_LIGHT}`, {
                type: 'channel',
                common: {
        ...
        await this._adapter.upsertState(`${this.liveStreamChannelId}.${constants_1.STATE_ID_LIVESTREAM_DURATION}`, constants_1.COMMON_LIVESTREAM_DURATION, this._durationLiveStream, true);
        await this._adapter.upsertState(`${this.eventsChannelId}.ondemand`, constants_1.COMMON_ON_DEMAND, false);
        await this._adapter.upsertState(`${this.snapshotChannelId}.auto`, constants_1.COMMON_SNAPSHOT_AUTO, this._adapter.config.auto_snapshot);
        await this._adapter.upsertState(`${this.HDsnapshotChannelId}.auto`, constants_1.COMMON_HDSNAPSHOT_AUTO, this._adapter.config.auto_HDsnapshot);
        await this._adapter.upsertState(`${this.liveStreamChannelId}.auto`, constants_1.COMMON_LIVESTREAM_AUTO, this._adapter.config.auto_livestream);
        // Remove legacy states
        await this._adapter.delObjectAsync(`${this.snapshotChannelId}.snapshot_file`);
        await this._adapter.delObjectAsync(`${this.snapshotChannelId}.snapshot_url`);
        await this._adapter.delObjectAsync(`${this.HDsnapshotChannelId}.snapshot_file`);
        await this._adapter.delObjectAsync(`${this.HDsnapshotChannelId}.snapshot_url`);
        await this._adapter.delObjectAsync(`${this.liveStreamChannelId}.livestream_file`);
        await this._adapter.delObjectAsync(`${this.liveStreamChannelId}.livestream_url`);
    }
    ...
    getActiveNightImageOptions() {
        ...
        if (this._adapter.Sunrise > 0 && this._adapter.Sunset > 0) {
            ...
            night_contrast =
                (this._adapter.config.night_contrast_HDsnapshot && isNight) ||
                    !this._adapter.config.night_contrast_HDsnapshot;
            night_sharpen =
                (this._adapter.config.night_sharpen_HDsnapshot && isNight) ||
                    !this._adapter.config.night_sharpen_HDsnapshot;
        }
    ...
    updateByDevice(ringDevice) {
        ...
        this.update(ringDevice.data).catch(e => this.catcher("Couldn't update device data.", e));
    }
    async update(data) {
    ...
    setDurationLivestream(val) {
        ...
        this._adapter
            .upsertState(`${this.liveStreamChannelId}.${constants_1.STATE_ID_LIVESTREAM_DURATION}`, constants_1.COMMON_LIVESTREAM_DURATION, this._durationLiveStream)
            .catch(e => this.catcher(`Cannot upsert state ${this.liveStreamChannelId}.${constants_1.STATE_ID_LIVESTREAM_DURATION}`, e));
    ...
    updateHealth() {
        ...
        this._ringDevice
            .getHealth()
            .then(this.updateHealthObject.bind(this))
            .catch(e => this.catcher("Couldn't update Health.", e));
    }
    autoSched() {
        const media = [
            {
                ...
                fct: () => {
                    this.takeSnapshot().catch(e => this.catcher('Cannot take snapshot', e));
                },
                start: 0,
            },
            {
                ...
                fct: () => {
                    this.takeHDSnapshot().catch(e => this.catcher('Cannot take HD snapshot', e));
                },
                start: 20,
            },
            {
                ...
                fct: () => {
                    this.startLivestream(this._adapter.config.recordtime_auto_livestream).catch(e => this.catcher('Cannot start livestream', e));
                },
                start: 40,
            },
        ];
        for (const m of media) {
            if (m.val > 0) {
                ...
                this._adapter.registerScheduledJob(node_schedule_1.default.scheduleJob(`Auto save ${m.name}_${this._adapter.name}_${this._adapter.instance}`, t, async () => {
                    ...
                    if (!recAct || !recAct.val) {
                        ...
                        this._adapter
                            .upsertState(`${this.eventsChannelId}.ondemand`, constants_1.COMMON_ON_DEMAND, true)
                            .catch(e => this.catcher(`Cannot upsert state ${this.eventsChannelId}.ondemand`, e));
    ...
    async subscribeToEvents() {
        ...
        this._ringDevice.onMotionDetected.subscribe({
            next: (motion) => {
                this.onMotion(motion).catch(e => this.catcher('Cannot react on motion', e));
            },
            error: (err) => {
        ...
        this._ringDevice.onDoorbellPressed.subscribe({
            next: (ding) => {
                this.onDoorbell(ding).catch(e => this.catcher('Cannot react on doorbell', e));
            },
            error: (err) => {
    ...
    async updateDeviceInfoObject(data) {
        ...
        await this._adapter.upsertState(`${this.infoChannelId}.external_connection`, constants_1.COMMON_INFO_EXTERNAL_CONNECTION, data.external_connection);
        await this._adapter.upsertState(`${this.infoChannelId}.hasLight`, constants_1.COMMON_INFO_HAS_LIGHT, this._ringDevice.hasLight);
        await this._adapter.upsertState(`${this.infoChannelId}.hasBattery`, constants_1.COMMON_INFO_HAS_BATTERY, this._ringDevice.hasBattery);
        await this._adapter.upsertState(`${this.infoChannelId}.hasSiren`, constants_1.COMMON_INFO_HAS_SIREN, this._ringDevice.hasSiren);
    }
    async updateHistoryObject(lastAction) {
        await this._adapter.upsertState(`${this.historyChannelId}.created_at`, constants_1.COMMON_HISTORY_CREATED_AT, lastAction.event.created_at);
        await this._adapter.upsertState(`${this.historyChannelId}.history_url`, constants_1.COMMON_HISTORY_URL, lastAction.historyUrl);
        await this._adapter.upsertState(`${this.historyChannelId}.kind`, constants_1.COMMON_HISTORY_KIND, lastAction.event.kind);
    }
    async updateSnapshotRequest(ack = true) {
        await this._adapter.upsertState(`${this.eventsChannelId}.ondemand`, constants_1.COMMON_ON_DEMAND, false);
        // subscribe to true, because user request doesn't change value via upsertState
        await this._adapter.upsertState(`${this.snapshotChannelId}.${constants_1.STATE_ID_SNAPSHOT_REQUEST}`, constants_1.COMMON_SNAPSHOT_REQUEST, false, ack, true);
    }
    async updateHDSnapshotRequest(ack = true) {
        await this._adapter.upsertState(`${this.eventsChannelId}.ondemand`, constants_1.COMMON_ON_DEMAND, false);
        // subscribe to true, because user request doesn't change value via upsertState
        await this._adapter.upsertState(`${this.HDsnapshotChannelId}.${constants_1.STATE_ID_HDSNAPSHOT_REQUEST}`, constants_1.COMMON_HDSNAPSHOT_REQUEST, false, ack, true);
    }
    async updateLivestreamRequest(ack = true) {
        await this._adapter.upsertState(`${this.eventsChannelId}.ondemand`, constants_1.COMMON_ON_DEMAND, false);
        // subscribe to true, because user request doesn't change value via upsertState
        await this._adapter.upsertState(`${this.liveStreamChannelId}.${constants_1.STATE_ID_LIVESTREAM_REQUEST}`, constants_1.COMMON_LIVESTREAM_REQUEST, false, ack, true);
        this._durationLiveStream = this._adapter.config.recordtime_livestream;
        await this._adapter.upsertState(`${this.liveStreamChannelId}.${constants_1.STATE_ID_LIVESTREAM_DURATION}`, constants_1.COMMON_LIVESTREAM_DURATION, this._durationLiveStream, ack);
    }
    async updateSnapshotObject() {
        this.debug(`Update Snapshot Object`);
        if (this._lastSnapshotTimestamp !== 0) {
            await this._adapter.upsertState(`${this.snapshotChannelId}.file`, constants_1.COMMON_SNAPSHOT_FILE, this._lastSnapShotDir);
            await this._adapter.upsertState(`${this.snapshotChannelId}.moment`, constants_1.COMMON_SNAPSHOT_MOMENT, this._lastSnapshotTimestamp);
            await this._adapter.upsertState(`${this.snapshotChannelId}.url`, constants_1.COMMON_SNAPSHOT_URL, this._lastSnapShotUrl);
        }
    ...
    async updateHDSnapshotObject() {
        ...
        if (this._lastHDSnapshotTimestamp !== 0) {
            await this._adapter.upsertState(`${this.HDsnapshotChannelId}.file`, constants_1.COMMON_HDSNAPSHOT_FILE, this._lastHDSnapShotDir);
            await this._adapter.upsertState(`${this.HDsnapshotChannelId}.moment`, constants_1.COMMON_HDSNAPSHOT_MOMENT, this._lastHDSnapshotTimestamp);
            await this._adapter.upsertState(`${this.HDsnapshotChannelId}.url`, constants_1.COMMON_HDSNAPSHOT_URL, this._lastHDSnapShotUrl);
        }
    ...
    async updateLiveStreamObject() {
        ...
        if (this._lastLiveStreamTimestamp !== 0) {
            await this._adapter.upsertState(`${this.liveStreamChannelId}.file`, constants_1.COMMON_LIVESTREAM_FILE, this._lastLiveStreamDir);
            await this._adapter.upsertState(`${this.liveStreamChannelId}.url`, constants_1.COMMON_LIVESTREAM_URL, this._lastLiveStreamUrl);
            await this._adapter.upsertState(`${this.liveStreamChannelId}.moment`, constants_1.COMMON_LIVESTREAM_MOMENT, this._lastLiveStreamTimestamp);
        }
        await this.updateLivestreamRequest();
    }
    async updateHealthObject(health) {
        this.debug('Update Health Callback');
        let batteryPercent = parseInt(health.battery_percentage ?? '-1');
        if (isNaN(batteryPercent)) {
            batteryPercent = -1;
        }
        await this._adapter.upsertState(`${this.infoChannelId}.battery_percentage`, constants_1.COMMON_INFO_BATTERY_PERCENTAGE, batteryPercent);
        await this._adapter.upsertState(`${this.infoChannelId}.battery_percentage_category`, constants_1.COMMON_INFO_BATTERY_PERCENTAGE_CATEGORY, health.battery_percentage_category);
        await this._adapter.upsertState(`${this.infoChannelId}.wifi_name`, constants_1.COMMON_INFO_WIFI_NAME, health.wifi_name);
        await this._adapter.upsertState(`${this.infoChannelId}.latest_signal_strength`, constants_1.COMMON_INFO_LATEST_SIGNAL_STRENGTH, health.latest_signal_strength);
        await this._adapter.upsertState(`${this.infoChannelId}.latest_signal_category`, constants_1.COMMON_INFO_LATEST_SIGNAL_CATEGORY, health.latest_signal_category);
        await this._adapter.upsertState(`${this.infoChannelId}.firmware`, constants_1.COMMON_INFO_FIRMWARE, health.firmware);
        if (this._ringDevice.hasLight && Date.now() - this._lastLightCommand > 60000) {
            ...
            await this._adapter.upsertState(`${this.lightChannelId}.light_state`, constants_1.COMMON_LIGHT_STATE, floodlightOn);
        }
    }
    async onNotify(value) {
        ...
        if (value) {
            ...
            if (value.img != null) {
                await this.notifyRecording(EventState.ReactingOnEvent, value.img.snapshot_uuid);
            }
            const subType = value.data.event.ding.subtype;
            await this._adapter.upsertState(`${this.eventsChannelId}.type`, constants_1.COMMON_EVENTS_TYPE, text_service_1.TextService.getDetectionType(subType, this._adapter.language));
            await this._adapter.upsertState(`${this.eventsChannelId}.detectionType`, constants_1.COMMON_EVENTS_DETECTIONTYPE, text_service_1.TextService.getDetectionType(value.data.event.ding.detection_type ?? subType, this._adapter.language));
            await this._adapter.upsertState(`${this.eventsChannelId}.created_at`, constants_1.COMMON_EVENTS_MOMENT, Date.now());
            await this._adapter.upsertState(`${this.eventsChannelId}.message`, constants_1.COMMON_EVENTS_MESSAGE, value.android_config.body);
        }
    }
    async onMotion(value) {
        // value = true -> motion
        ...
        if (value) {
            ...
            await this._adapter.upsertState(`${this.eventsChannelId}.motion`, constants_1.COMMON_MOTION, value);
        }
    }
    async onDoorbell(value) {
        ...
        if (value) {
            ...
            await this._adapter.upsertState(`${this.eventsChannelId}.doorbell`, constants_1.COMMON_EVENTS_DOORBELL, true);
            this._adapter.setTimeout(() => {
                this._adapter
                    .upsertState(`${this.eventsChannelId}.doorbell`, constants_1.COMMON_EVENTS_DOORBELL, false)
                    .catch(e => this.catcher(`Cannot upsert state ${this.eventsChannelId}.doorbell`, e));
            }, 1000);
        }
    }
    async notifyRecording(state, uuid) {
      ...
      if (this._adapter.config.auto_snapshot)
        if (subscr && !this._ringDevice.hasBattery)
      ...
      */
            ...
            this._adapter.config.auto_livestream &&
                (await this.startLivestream(this._adapter.config.recordtime_auto_livestream));
            // give some time to evaluate motion state, e.g. for node-red
            this._adapter.setTimeout(() => {
                this._adapter
                    .upsertState(`${this.eventsChannelId}.motion`, constants_1.COMMON_MOTION, false)
                    .catch(e => this.catcher(`Cannot set state ${this.eventsChannelId}.motion to false`, e));
            }, 200);
