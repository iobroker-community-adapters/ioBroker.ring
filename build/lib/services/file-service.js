...
class FileService {
    static IOBROKER_FILES_REGEX = new RegExp(/.*iobroker-data\/files.*/);
    ...
    static getPath(basePath, extendedPath, count, shortId, fullId, kind) {
        const fullPath = node_path_1.default
            .join(basePath, fullId, extendedPath)
        ...
        return {
            fullPath: fullPath,
            dirname: node_path_1.default.dirname(fullPath),
            filename: node_path_1.default.basename(fullPath),
        };
    }
        ...
        return new Promise((resolve) => {
            if (!node_fs_1.default.existsSync(dirname)) {
                ...
                if (!main_1.RingAdapter.isWindows) {
                    node_fs_1.default.chmodSync(dirname, 508);
                }
            }
            resolve(true);
        });
    ...
    static async getVisUrl(adapter, fullId, fileName) {
        const vis = await adapter
            .getForeignObjectAsync('system.adapter.web.0')
            .catch((reason) => {
    ...
    static async getTempDir(adapter) {
        const tempPath = node_path_1.default.join(adapter.absoluteInstanceDir);
    ...
    static async writeFile(fullPath, data, adapter) {
        return new Promise((resolve, reject) => {
            if (!this.IOBROKER_FILES_REGEX.test(fullPath)) {
                node_fs_1.default.writeFile(fullPath, data, (r) => {
                    if (r) {
